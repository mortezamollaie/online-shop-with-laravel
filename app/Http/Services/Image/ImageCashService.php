...
class ImageCacheService{
    public function cache($imagePath, $size = ''){
        ...
        $heigth = $imageSizes[$size]['height'];

        //cache image
        if(file_exists($imagePath)){
            $img = Image::cache(function($image) use($imagePath, $width, $heigth){
                return $image->make($imagePath)->fit($width, $heigth);
            }, Config::get('image-cache-life-time'), true);
            return $img->response();
        } else {
            $img = Image::canvas($width, $heigth, '#cdcdcd')->text('image not found - 404', $width/2 , $heigth/2, function($font){
                $font->color('#333333');
                $font->align('center');
                $font->valign('center');
